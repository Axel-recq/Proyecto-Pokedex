...
useEffect(() => {
  ...
  if (especiePokemon?.url_especie) {
    const obtenerEvoluciones = async () => {
      ...
      // 1. Obtener la URL de la cadena de evolución
      const URL = especiePokemon?.url_especie?.url.split("/");
      // Nota: Asegúrate de tener importada la constante URL_EVOLUCIONES o usa la URL completa
      const api = await axios.get(`${URL_EVOLUCIONES}/${URL[6]}`);
      
      // 2. Primera Evolución (Base)
